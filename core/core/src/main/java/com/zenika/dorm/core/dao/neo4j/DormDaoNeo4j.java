...
//        }
    ...
    private DependencyNode getDependencyNodeChildren(DependencyNode node) {
        ...
        try {
            ...
            dependency = searchNode(Neo4jMetadata.generateIndexURI(metadata.getName(), index), type.getType());
            ...
            if (e.getResponse().getStatus() == Response.Status.NOT_FOUND.getStatusCode()) {
                LOG.debug("The dependency node with this " + metadata.getName() + " full qualifier doesn't " +
                        "found");
            }
        } catch (URISyntaxException e) {
            throw new Neo4jDaoException("URI syntax exception", e);
